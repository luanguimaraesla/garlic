...
func TestHasUnitBuildTag(t *testing.T) {
	cases := []struct {
		...
		{"unit and negated goos", "//go:build unit && !windows", true},
		{"unit and unix", "//go:build unit && unix", true},
		{"unit and cgo", "//go:build unit && cgo", true},
		{"unit or negated goos", "//go:build unit || !linux", false},
